...
<header>
    ...
    <main>
        <div class="row">
                ...
                <div class="row">
                    <div class="col l12">
                        <table class="centered responsive-table card">
                            ...
                            $sql=$connect->prepare("SELECT tabsetor.idsetor, tabsetor.nomesetor, tabcorredor.nomecorredor, tabsetor.idcorredor 
                                    FROM tabsetor 
                                INNER JOIN tabcorredor ON tabcorredor.idcorredor = tabsetor.idcorredor");
                            $sql->execute();
                            ...
                            if($result->num_rows == 0){
                              ...
                              while($res= $result->fetch_array()){
                                ?>
                                <tr>
                                    <td><?php echo $res['idsetor'] ?></td>
                                    <td><?php echo $res['nomesetor'] ?></td>
                                    <td><?php echo $res['nomecorredor'] ?></td>
                                    <td class='edita'><a href="#edita<?php echo $res['idsetor'] ?>"
                                            class='modal-trigger orange-text text-darken-3'><i
                                                class="material-icons">create</i></a></td>
                                </tr>

                                <div class='modal fade' id='edita<?php echo $res['idsetor'] ?>'>
                                    <div class='modal-content'>
                                        <div class='row right'><i class='material-icons modal-close'>close</i></div>
                                        <h4>Editar informações</h4>
                                        <div class='divider'></div>
                                        <form method='post'
                                            action='../controller/update/editaSetor.php?id=<?php echo $res['idsetor'] ?>'
                                            id='form-edita<?php echo $res['idsetor'] ?>'>
                                            <div class="row">
                                                <div class="input-field col l6">
                                                    <input id="nome-setor<?php echo $res['idsetor'] ?>" type="text"
                                                        class="validate" name='nome-setor'
                                                        value='<?php echo $res['nomesetor']?>'
                                                        onkeyup="this.value = this.value.toUpperCase();">
                                                    <label for="nome-produto<?php echo $res['idsetor'] ?>">Nome do
                                                        setor</label>
                                                </div>
                                                <div class="input-field col l6">
                                                    <select name='corredor-setor'
                                                        id='corredor-setor<?php echo $res['idsetor'] ?>'>
                                                        <option value="<?php echo $res['idcorredor'] ?>" selected>
                                                            <?php echo $res['nomecorredor'] ?></option>
                                                        <?php
                                                $sqlCorredor = "SELECT * FROM tabcorredor";
                                                $resultCorredor = $connect->query($sqlCorredor);
                                                while($resCorredor = $resultCorredor->fetch_assoc()){
                                                    ?>
                                                        <option value='<?php echo $resCorredor['idcorredor'] ?>'>
                                                            <?php echo $resCorredor['nomecorredor'] ?></option>
                                                        <?php
                                                }
                                                    ?>
                                                    </select>
                                            ...
                                            <div class="row center">
                                                <div class='btn light-blue darken-3 envia' type='submit' name='action'
                                                    id='<?php echo $res['idsetor'] ?>'>Atualizar
            ...
            <div class="col l2" style='margin-top: 1%;'>
                <div class="row center">
                    <button class="btn light-blue darken-3 editar" id='<?php echo $res['idsetor'] ?>'
    ...
    $(document).ready(function() {
        ...
        $(".envia").click(function() {
            var id = $(this).attr("id");
            var corredor = $("#corredor-setor" + id).val();
            var url = $("#form-edita" + id).attr("action");

            url += "&corredor-setor=" + corredor;

            $("#form-edita" + id).attr("action", url);
            $("#form-edita" + id).submit();

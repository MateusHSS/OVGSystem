    ...
    if($_FILES['formulario_pedido']['name'] != ''){
        ...
        if(move_uploaded_file($_FILES['formulario_pedido']['tmp_name'], $pastaArq) ){
            ...
            if(isset($_POST['emergencial'])){
                ...
                $sqlAtualizaTabPedido = $connect->prepare("UPDATE tabpedido SET quantidadepedido = ?, altura = ?, largura = ?, atividade = ?, seguranca = ?, formulariopedido = ?, emergencial = ? WHERE idpedido = ?");
                $sqlAtualizaTabPedido->bind_param('sssssssi', $qtd, $alt, $larg, $atividade, $seguranca, $nomeArq, $id, $emerg);
                $sqlAtualizaTabPedido->execute();
            
            }else{
                
                //ATUALIZA A TABELA DO PEDIDO
                $sqlAtualizaTabPedido = $connect->prepare("UPDATE tabpedido SET quantidadepedido = ?, altura = ?, largura = ?, atividade = ?, seguranca = ?, formulariopedido = ? WHERE idpedido = ?");
                $sqlAtualizaTabPedido->bind_param('sssssss', $qtd, $alt, $larg, $atividade, $seguranca, $nomeArq, $id);
                $sqlAtualizaTabPedido->execute();
            }
            ...
            if($sqlAtualizaTabPedido->affected_rows > 0){
                ...
                if($status['nomestatus'] == 'PROGRAMADO'){
                    ...
                    if(isset($_POST['emergencial'])){
                        ...
                        //ATUALIZA A TABELA DO PEDIDO
                        $sqlAtualizaTabPedido = $connect->prepare("UPDATE tabpedido SET statuspedido = ? WHERE idpedido = ?");
                        $sqlAtualizaTabPedido->bind_param('ii', $status);
                        $sqlAtualizaTabPedido->execute();
        ...
        if(isset($_POST['emergencial'])){
            ...
            $status = 3;
            //ATUALIZA A TABELA DO PEDIDO
            $sqlAtualizaTabPedido2 = $connect->prepare("UPDATE tabpedido SET quantidadepedido = ?, altura = ?, largura = ?, atividade = ?, seguranca = ?, emergencial = ? WHERE idpedido = ?");
            $sqlAtualizaTabPedido2->bind_param('ssssssi', $qtd, $alt, $larg, $atividade, $seguranca, $emerg, $id);
            $sqlAtualizaTabPedido2->execute();
        
        }else{
            //ATUALIZA A TABELA DO PEDIDO
            $sqlAtualizaTabPedido2 = $connect->prepare("UPDATE tabpedido SET quantidadepedido = ?, altura = ?, largura = ?, atividade = ?, seguranca = ? WHERE idpedido = ?");
            $sqlAtualizaTabPedido2->bind_param('ssssss', $qtd, $alt, $larg, $atividade, $seguranca, $id);
            $sqlAtualizaTabPedido2->execute();
        }

        if($sqlAtualizaTabPedido2->affected_rows > 0){
            //VERIFICA SE O PEDIDO JA HAVIA SIDO PROGRAMADO
            $sqlVerificaProgramacao = $connect->prepare("SELECT tabpedido.statuspedido, tabstatus.nomestatus FROM tabpedido INNER JOIN tabstatus ON tabstatus.idtabstatus = tabpedido.statuspedido WHERE tabpedido.idpedido = $id");
            $sqlVerificaProgramacao->execute();
            $status = $sqlVerificaProgramacao->get_result()->fetch_assoc();
            ...
            if($status['nomestatus'] == 'PROGRAMADO'){
                ...
                if(isset($_POST['emergencial'])){
                    ...
                    //ATUALIZA A TABELA DO PEDIDO
                    $sqlAtualizaTabPedido = $connect->prepare("UPDATE tabpedido SET statuspedido = ? WHERE idpedido = ?");
                    $sqlAtualizaTabPedido->bind_param('ii', $status, $id);
                    $sqlAtualizaTabPedido->execute();
            ...
            echo json_encode(array("cod" => '0', "n" => "5", "erro" => $sqlAtualizaTabPedido2->error));
        }
    }

                ...
                if($aux_id_processo != $processo["idtabprocessosproduto"]){
                    ...
                    if($inicio != NULL and $fim != NULL){
                        ...
                        if($calculo>=$processo["tempo"]){
                            ...
                            $sql_insert_subprocesso = "INSERT INTO `tabprocessosproduto`(`idproduto`, `idprocesso`, `vezes`, `funcionarios`, `tempo`, `ordem`) VALUES 
                                (?,?,?,?,?,?)";
                            $insert = $connect->prepare($sql_insert_subprocesso);
                            $insert->bind_param('ssssss', $processo['idproduto'], $processo['idprocesso'], $processo['vezes'], $processo['funcionarios'], $novo_tempo, $processo['ordem']);
                            $insert->execute();
